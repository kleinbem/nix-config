...
pc_assert_safe_target() {
    ...
    [ -b "$dev" ] || { echo "❌ SAFETY ABORT: $dev is not a block device."; exit 1; }
    tran=$(lsblk -dno TRAN "$dev" 2>/dev/null)
    if [ "$tran" != "usb" ]; then
        echo "❌ SAFETY ABORT: $dev is not USB-attached (TRAN=$tran). Refusing to wipe."; exit 1
    fi
    d=$(basename "$dev")
    ...
    if [ "$d" = "$rootpk" ] || [ "$d" = "$bootpk" ]; then
        echo "❌ SAFETY ABORT: $dev backs THIS host's / ($rootpk) or /boot ($bootpk)."; exit 1
    fi
...
pc_require_block_device() {
    local dev="$1"
    [ -b "$dev" ] || { echo "❌ Error: $dev is not a valid block device (check lsblk)."; exit 1; }
...
pc_cache_preflight() {
    local host="$1" extra="${2:-}"; shift $(( $# >= 2 ? 2 : $# ))
