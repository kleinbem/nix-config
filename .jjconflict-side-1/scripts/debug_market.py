...
def search_marketplace(query, use_openvsx=False):
    if use_openvsx:
        ...
        data = {
            "filters": [{"criteria": [{"filterType": 10, "value": query}]}],
            "flags": 914,
        }
        ...
        try:
            ...
            for e in exts:
                ...
                display_name = e.get("displayName", "No display name")
                version = (
                    e["versions"][0]["version"] if e.get("versions") else "Unknown"
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Search VS Code Marketplace or Open VSX for extensions."
    )
    parser.add_argument(
        "query", help="Search term (e.g., publisher name, extension name, or keyword)"
    )
    parser.add_argument(
        "--openvsx",
        action="store_true",
        help="Search Open VSX Registry instead of VS Code Marketplace",
    )
