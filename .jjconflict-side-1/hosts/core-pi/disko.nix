# core-pi disko layout — Raspberry Pi 5 on native NVMe (PCIe HAT).
#
# No on-disk swap: core-pi's job is to run AI & Infrastructure services, and all heavy
...
# a btrfs swapfile inside core_crypt).
{
  device ? "/dev/nvme0n1",
  ...
}:
